...
class SitemapDataGrid extends DataGrid
{
    ...
    {
        if (bouncer_manage()->hasPermission('marketing.sitemaps.edit')) {
            $this->addAction([
                'icon'   => 'icon-edit',
                ...
                'url'    => function ($row) {
                    return route('manage.marketing.promotions.sitemaps.update', $row->id);
                },
            ]);
        }

        if (bouncer_manage()->hasPermission('marketing.sitemaps.delete')) {
            $this->addAction([
                'icon'   => 'icon-delete',
                'title'  => trans('admin::app.marketing.sitemaps.index.datagrid.delete'),
                'method' => 'DELETE',
                'url'    => function ($row) {
                    return route('manage.marketing.promotions.sitemaps.delete', $row->id);
                },
            ]);
        }
    }
