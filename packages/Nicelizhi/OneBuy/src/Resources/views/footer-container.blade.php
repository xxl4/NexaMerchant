<div class="footer-container">
<div class="copyright-block">
©2016- Hatme
</div>
<style>
    .phone-block {
        color: #000;
        font-size: 14px;
        font-family: Helvetica Bold;
        margin: 0 0 15px;
        text-align: center;
        padding: 5px 0;
    }
    .phone-block a {
        color: #a9a9a9;
        font-size: 14px;
    }
    .terms-block a{
        color: #FFF;
        font-weight:blod;
    }
</style>
<div class="phone-block">
     </div>
    <div class="terms-block">
            <a href="/onebuy/page/shipping-policy?locale={{ app()->getLocale() }}" target="_blank">
         @lang('onebuy::app.product.footer.Shipping Delivery') </a>
            <a href="/onebuy/page/refund-policy?locale={{ app()->getLocale() }}" target="_blank">
         @lang('onebuy::app.product.footer.Refund policy')  </a>
            <a href="/onebuy/page/about-us?locale={{ app()->getLocale() }}" target="_blank" >
         @lang('onebuy::app.product.footer.About US')</a>
            <a href="/onebuy/page/privacy-policy?locale={{ app()->getLocale() }}" target="_blank" >
         @lang('onebuy::app.product.footer.Privacy Policy') </a>
            <a href="/onebuy/page/contact-us?locale={{ app()->getLocale() }}" target="_blank">
         @lang('onebuy::app.product.footer.Contact Us')  </a>
            <a href="/onebuy/page/Impressum?locale={{ app()->getLocale() }}" target="_blank" >Impressum</a>
    </div>
    <div class="dmca_logo">
      <a href="//www.dmca.com/Protection/Status.aspx?ID=12ec7509-f986-41d6-81e0-78ae55efbf30" title="DMCA.com Protection Status" class="dmca-badge" target="_blank"> <img src ="https://images.dmca.com/Badges/dmca_protected_sml_120n.png?ID=12ec7509-f986-41d6-81e0-78ae55efbf30"  alt="DMCA.com Protection Status" /></a>  <script src="https://images.dmca.com/Badges/DMCABadgeHelper.min.js"> </script>
    </div>

    
</div>