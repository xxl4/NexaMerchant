                        ...
                        <x-slot:content>
                            <div class="flex flex-wrap gap-[29px] mt-[30px]">
                                ...
                                <template v-for="(payment, index) in payment_methods">
                                    <template v-if="payment.method === 'aaaa'">
        ...
        app.component('v-payment-method', {
            ...
            methods: {
                store(selectedPaymentMethod) {
                    this.$axios.post("{{ route('shop.checkout.onepage.payment_methods.store') }}", {
                        ...
                        .then(response => {
                            ...
                            this.$axios.post("{{ route('shop.checkout.onepage.airwallex') }}", {
                ...
                getMultiSafePaymentMethods() {
                        this.$axios.get("{{ route('airwallex.payment_methods')}}")
                        .then(response => {
                            this.MultiSafePaymentMethods = response.data;
                            
                            console.log('PaymentMethods:', this.MultiSafePaymentMethods);
                        })
                        .catch(error => console.log(error));
                    }
            },
